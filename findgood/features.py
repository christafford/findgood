...
def compute_features(trade_date: date, min_price: float = 1.0,
    ...
    Requires trade_date to exist in day_aggs (for backtesting).
    Returns list of dicts with feature values and actual intraday return.
    """
    conn = get_connection()
    try:
        with conn.cursor() as cur:
...
def compute_features_forward(min_price: float = 1.0,
                             min_avg_volume: float = 100_000) -> list[dict]:
    """Compute features for the NEXT trading day using the latest available data.

    Unlike compute_features(), this does NOT require the target day to exist
    in day_aggs. It uses the most recent trading day as 'yesterday' and
    computes features that would be available at tomorrow's market open.

    Returns list of dicts with feature values (no intraday_return since
    tomorrow hasn't happened yet).
    """
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(FORWARD_FEATURE_QUERY, {
                "min_price": min_price,
                "min_avg_volume": min_avg_volume,
            })
            columns = [desc[0] for desc in cur.description]
...
# Forward-looking query: uses the latest available day as "yesterday"
# and computes features for the NEXT trading day (which doesn't exist yet).
# No intraday_return since we're predicting, not backtesting.
FORWARD_FEATURE_QUERY = """
WITH latest_date AS (
    SELECT MAX(trade_date) AS dt FROM day_aggs
),

-- "yesterday" is the latest day in our data
yesterday AS (
    SELECT d.ticker_id, t.symbol, d.open, d.close, d.high, d.low, d.volume
    FROM day_aggs d
    JOIN tickers t ON d.ticker_id = t.id
    JOIN latest_date ld ON d.trade_date = ld.dt
    WHERE d.open > 0 AND d.close > 0
),
...
prior_stats AS (
    SELECT
        ...
        COUNT(*) FILTER (WHERE p.days_ago <= 10) AS trading_days_10d,
        MAX(CASE WHEN p.days_ago = 1 THEN p.trade_date END) AS prev_trade_date,
        MAX(CASE WHEN p.days_ago = 2 THEN p.trade_date END) AS prev_trade_date_2
    FROM prior p
    GROUP BY p.ticker_id
),
...
spy_prior AS (
    ...
    FROM day_aggs d
    JOIN tickers t ON d.ticker_id = t.id
    JOIN latest_date ld ON true
    WHERE t.symbol = 'SPY'
      AND d.trade_date <= ld.dt
      AND d.trade_date >= ld.dt - 30
),
...
news_stats AS (
    ...
    JOIN sentiments s ON ns.sentiment_id = s.id
    JOIN latest_date ld ON true
    WHERE ns.published_utc >= (ld.dt - 3)::timestamp
      AND ns.published_utc < (ld.dt + 1)::timestamp
    GROUP BY ns.ticker_id
)
...
SELECT
    ...
    0::float AS intraday_return,  -- unknown, placeholder

    0::float AS overnight_gap,  -- unknown until market opens
...
FROM yesterday y
JOIN prior_stats ps ON y.ticker_id = ps.ticker_id
LEFT JOIN eod_cache eod ON eod.ticker_id = y.ticker_id
                       AND eod.trade_date = ps.prev_trade_date_2
LEFT JOIN news_stats nw ON y.ticker_id = nw.ticker_id
LEFT JOIN stock_spy_corr ssc ON y.ticker_id = ssc.ticker_id
LEFT JOIN stock_sector_corr scc ON y.ticker_id = scc.ticker_id
LEFT JOIN spy_yesterday sy ON true

WHERE y.close >= %(min_price)s
  AND COALESCE(ps.avg_volume_10d, 0) >= %(min_avg_volume)s
  AND ps.trading_days_10d >= 5

ORDER BY y.ticker_id
"""
